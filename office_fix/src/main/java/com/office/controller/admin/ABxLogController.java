...
@CrossOrigin(origins = "*")
@Controller
@RequestMapping("/admin/bx_log")
public class ABxLogController{
	...
	public Map<String,Object> getList(Map<String,Object> rs ,BxLog model,HttpServletRequest request){
		...
		List<Map<String,Object>> bgServiceList = bgServiceService.getSelectList(new BgService(),new ArrayList<Map<String,Object>>(), login);
		rs.put("bgServiceList",bgServiceList);
		data.put("bgServiceList",bgServiceList);
		...
		List<Map<String,Object>> fixList = fixService.getSelectList(new Fix(),new ArrayList<Map<String,Object>>(), login);
		rs.put("fixList",fixList);
		data.put("fixList",fixList);
		return data;
	}
	/**
	* 根据查询条件分页查询报修记录数据总数
	...
	*/
	...
	派单提交信息接口
	*/
	@RequestMapping("pd_submit")
	@ResponseBody
	public Object pd_submit(BxLog model,ModelMap modelMap,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> rs = new HashMap<String,Object>();
		String msg = bxLogService.pd(model,login);//执行修改操作
		if(msg.equals("")==false){
			rs.put("code",0);
			rs.put("msg",msg);
			return rs;
	...
	驳回提交信息接口
	*/
	@RequestMapping("bh_submit")
	@ResponseBody
	public Object bh_submit(BxLog model,ModelMap modelMap,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> rs = new HashMap<String,Object>();
		String msg = bxLogService.bh(model,login);//执行修改操作
		if(msg.equals("")==false){
			rs.put("code",0);
			rs.put("msg",msg);
			return rs;
