Page({
	...
	initDetail(){
		var that = this;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/index/bg_service_detail/getBgServiceDetail',
			method: 'POST',
			data: {
	...
	*/
	...
	getBxLogData(){
		var that = this;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/index/bx_log/getListData',
			method: 'POST',
			data: {
			...
			success: function(res) {
				...
				if(res.data.code==0){
					...
					that.setData({
						byxcdList:that.castListVal(res.data.yxcdList),
					});
					that.updateShowList('byxcd');
				}
			},
			fail: function(err) {
				console.error('请求失败', err);
	...
	submitBxLog(){
		var that = this;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/index/bx_log/createBxLog',
			method: 'POST',
			data: {
				"token":wx.getStorageSync('token'),
				"yxcd":that.data.byxcd==undefined?'':that.data.byxcd,
				"gzms":that.data.bgzms==undefined?'':that.data.bgzms,
				"gzImg":that.data.bgzImg==undefined?'':that.data.bgzImg,
				"gzVeo":that.data.bgzVeo==undefined?'':that.data.bgzVeo,
				"gzLocation":that.data.bgzLocation==undefined?'':that.data.bgzLocation,
				"bgService":that.data.id
			},
			header: {
				'content-type': 'application/x-www-form-urlencoded'
			},
	...
	async onShareAppMessage() {
		const app = getApp();
		return await app.getShareData(this, '办公室报修系统：办公设备详情');
	}
})
