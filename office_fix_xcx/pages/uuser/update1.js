Page({
	...
	submitData(){
		var that = this;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/user/user/update1_submit',
			method: 'POST',
			data: {
		...
		wx.request({
			...
			success: function(res) {
				...
				if(res.data.code==0){
					...
					that.initFileData(res.data.detail.user.headImg,'headImg');
					that.setData({
						name:res.data.detail.user.name,
						celphone:res.data.detail.user.celphone,
					depart:res.data.detail.user.depart});
				}
			},
			fail: function(err) {
				console.error('请求失败', err);
	...
	*/
	...
	getPageData(){
		var that = this;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/user/user/getRelativeDataList',
			method: 'POST',
			data: {
				"loginToken":wx.getStorageSync('token'),
			},
