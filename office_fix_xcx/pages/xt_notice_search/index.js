...
Page({
	...
	getXtNoticeList(){
		...
		var pageCount = this.getXtNoticeListCount();
	...
	getXtNoticeListCount(){
		var that = this;
		var totalPage = null;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/index/xt_notice_search/getXtNoticeCount',
			method: 'POST',
			data: {
	...
	getXtNoticeData(){
		var that = this;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/index/xt_notice_search/getXtNoticeList',
			method: 'POST',
			data: {
	...
	async onShareAppMessage() {
		const app = getApp();
		return await app.getShareData(this, '办公室报修系统：系统公告');
	}
})
