Page({
	...
	submitData(){
		var that = this;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/user/eval_log/update_submit',
			method: 'POST',
			data: {
	...
	initPreData(){
		var that = this;
		wx.request({
			url: wx.getStorageSync('baseHostPort')+'/user/eval_log/getDetailData',
			method: 'POST',
			data: {
				"id":that.data.id,
				"loginToken":wx.getStorageSync('token')
			...
			success: function(res) {
				...
				if(res.data.code==0){
					...
					that.setData({
						evalStar:res.data.detail.evalLog.evalStar,
					evalContent:res.data.detail.evalLog.evalContent});
				}
			},
			fail: function(err) {
				console.error('请求失败', err);
