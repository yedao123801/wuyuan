			...
			<template v-for="item in dataList">
				<tr  class="table-row">
					...
					<td class="list-table-action-td">
						<a href="javascript:void(0)" :data-id="item.evalLog.id" @click="del"  class="btnStyle1 btn btn-danger" >删除</a>
					</td>
				</tr>
			</template>
...
export default {
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/eval_log/getInitData",
				async: false,
				data : {
		...
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/eval_log/del",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/eval_log/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"bxid":bxid,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/eval_log/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
