	...
	<div id="body" class="edit-body" >
		<div class="edit-title-div">
			<div class="edit-title">
				<div class="edit-title-txt">维修员-修改</div>
			</div>
		</div>
		<div class="edit-main">
...
export default {
	...
	mounted() {
		...
		setZoneImg(fixZzDrop,that.fixZz);
	},
	methods:{
		queryDataDetail(){
			var that = this;
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/fix/queryDataDetail",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					...
					that.szType=result.szType;
					that.fixZz=result.fixZz;
				}
			});
		},
		submitData(){
			...
			$.ajax({
				type : 'post',
				url : this.REQUEST_URL+"/admin/fix/update_submit",
				async: false,
				data : {
					'id':that.id,
					'loginName':loginName,
