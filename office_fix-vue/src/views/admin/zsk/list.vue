...
<template>
	...
	<div class="list-global-btns">
		<a href="javascript:void(0)" :data-params="''" data-href="/admin/zsk/add_page" @click="goToPage"  class="btnStyle1 btn btn-success" >新增</a>
	</div>
</div>
				...
				<template v-for="item in dataList">
					<tr  class="table-row">
						...
						<td class="list-table-action-td">
							<a href="javascript:void(0)" :data-params="'id='+item.zsk.id+''" data-href="/admin/zsk/detail" @click="goToPage"  class="btnStyle1 btn btn-success" >查看详情</a>
							<a href="javascript:void(0)" :data-id="item.zsk.id" @click="del"  class="btnStyle1 btn btn-danger" >删除</a>
							<a href="javascript:void(0)" :data-params="'id='+item.zsk.id+''" data-href="/admin/zsk/update_page" @click="goToPage"  class="btnStyle1 btn btn-warning" >修改</a>
						</td>
					</tr>
				</template>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/zsk/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/zsk/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"title":title,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/zsk/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
