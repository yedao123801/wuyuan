...
<template>
	<div id="body" class="detail-body">
		...
		<div class="detail-title">
			<div class="detail-title-div">知识库</div>
		</div>
			...
			<div class="detail-main2">
				<div class="detail-msg-body">
					...
					<div class="detail-table-item-div1">
						<div class="detail-item">
							<div class="detail-table-col1">知识内容</div>
							<div class="detail-table-col2" v-html="zsContent==null?'':zsContent"></div>
						</div>
					</div>
					<div class="detail-table-item-div1">
						<div class="detail-item">
...
export default {
	...
	methods:{
		queryDataDetail(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/zsk/getDetailData",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					that.title = (result.detail.zsk.title==null?'':result.detail.zsk.title);
					that.subtitle = (result.detail.zsk.subtitle==null?'':result.detail.zsk.subtitle);
					that.zsContent = (result.detail.zsk.zsContent==null?'':result.detail.zsk.zsContent);
					that.fbTime = (result.detail.zsk.fbTime==null?'':result.detail.zsk.fbTime);
				}
			});
		},
	}
