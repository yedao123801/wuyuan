...
<template>
	...
	<div class="list-global-btns">
		<a href="javascript:void(0)" :data-params="''" data-href="/admin/xt_notice/add_page" @click="goToPage"  class="btnStyle1 btn btn-success" >新增</a>
	</div>
</div>
<div  class="list-table-container">
	<div class="table-responsive">
		<table class="list-table table table-bordered">
			...
			<tbody  id="xtNoticeList">
				<template v-for="item in dataList">
					<tr  class="table-row">
						<td >
							<div v-html="''+(item.xtNotice.title==null?'':item.xtNotice.title)+''" class="list-td-txt"></div>
						</td>
						<td >
							<div v-html="''+(item.xtNotice.subtitle==null?'':item.xtNotice.subtitle)+''" class="list-td-txt"></div>
						</td>
						<td >
							<div v-html="''+(item.xtNotice.fbTime==null?'':item.xtNotice.fbTime)+''" class="list-td-txt"></div>
						</td>
						<td class="list-table-action-td">
							<a href="javascript:void(0)" :data-params="'id='+item.xtNotice.id+''" data-href="/admin/xt_notice/detail" @click="goToPage"  class="btnStyle1 btn btn-success" >查看详情</a>
							<a href="javascript:void(0)" :data-id="item.xtNotice.id" @click="del"  class="btnStyle1 btn btn-danger" >删除</a>
							<a href="javascript:void(0)" :data-params="'id='+item.xtNotice.id+''" data-href="/admin/xt_notice/update_page" @click="goToPage"  class="btnStyle1 btn btn-warning" >修改</a>
						</td>
					</tr>
				</template>
		...
		<div class="list-page-utils">
			<div id="xt_notice_bar" class="pagination" style="margin-top:20px;"></div>
		</div>
	</div>
</div>
</div>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/xt_notice/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/xt_notice/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"title":title,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/xt_notice/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
