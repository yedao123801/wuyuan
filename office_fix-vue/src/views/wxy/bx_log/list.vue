				...
				<template v-for="item in dataList">
					<tr  class="table-row">
						...
						<td class="list-table-action-td">
							<a href="javascript:void(0)" :data-params="'id='+item.bxLog.id+''" data-href="/wxy/bx_log/detail" @click="goToPage"  class="btnStyle1 btn btn-success" >查看详情</a>
							<a href="javascript:void(0)" :data-params="'bxid='+item.bxLog.id+''" data-href="/wxy/fix_gc/list" @click="goToPage"  class="btnStyle1 btn btn-success" >查看维修过程</a>
							<template v-if="item.bxLog.bxStatus=='2'">
								<a href="javascript:void(0)" :data-id="item.bxLog.id" @click="wxz"  class="btnStyle1 btn btn-warning" >维修中</a>
							</template>
							<template v-if="(item.bxLog.bxStatus=='3') && (item.bxLog.fix==''+currentLoginId+'')">
								<a href="javascript:void(0)" :data-params="'id='+item.bxLog.id+''" data-href="/wxy/bx_log/wc_page" @click="goToPage"  class="btnStyle1 btn btn-success" >完成</a>
							</template>
						</td>
					</tr>
				</template>
...
export default {
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/wxy/bx_log/getInitData",
				async: false,
				data : {
		...
		wxz(e){
			...
			var r=confirm("确认维修中该数据吗？");
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/wxy/bx_log/wxz",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/wxy/bx_log/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"bxNo":bxNo,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/wxy/bx_log/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
