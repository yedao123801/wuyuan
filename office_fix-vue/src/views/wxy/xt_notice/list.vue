				...
				<template v-for="item in dataList">
					<tr  class="table-row">
						...
						<td class="list-table-action-td">
							<a href="javascript:void(0)" :data-params="'id='+item.xtNotice.id+''" data-href="/wxy/xt_notice/detail" @click="goToPage"  class="btnStyle1 btn btn-success" >查看详情</a>
						</td>
					</tr>
				</template>
...
export default {
	...
	methods:{
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/wxy/xt_notice/queryCount',
				async: false,
				data : {
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/wxy/xt_notice/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
