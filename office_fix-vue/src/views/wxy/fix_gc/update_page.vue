	...
	<div id="body" class="edit-body" >
		<div class="edit-title-div">
			<div class="edit-title">
				<div class="edit-title-txt">维修过程-修改</div>
			</div>
		</div>
		<div class="edit-main">
...
export default {
	...
	methods:{
		queryDataDetail(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/wxy/fix_gc/queryDataDetail",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					that.fixBz=result.fixBz;
					that.ghpj=result.ghpj;
					that.xhgs=result.xhgs;
				}
			});
		},
		submitData(){
			...
			$.ajax({
				type : 'post',
				url : this.REQUEST_URL+"/wxy/fix_gc/update_submit",
				async: false,
				data : {
					'id':that.id,
					'bxid':that.bxid,
